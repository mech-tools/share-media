...
async function _promptBoundingTileCreation(nextNumber) {
  ...
  return dialog({
    ...
    validateLabel: game.i18n.localize(
      `${constants.moduleName}.dialogs.bounding-tile-creation.create-button`
    ),
...
async function _promptClearBoundingTileSection(boundingTiles) {
  const boundingTilesData = boundingTiles
    .map((b) => ({ id: b.id, name: b.flags[constants.moduleName].name }))
  ...
  return dialog({
    ...
    title: game.i18n.localize(
      `${constants.moduleName}.dialogs.clear-bounding-tile-selection.title`
    ),
    content,
    cancelLabel: game.i18n.localize(
      `${constants.moduleName}.dialogs.clear-bounding-tile-selection.cancel-button`
    ),
    validateLabel: game.i18n.localize(
      `${constants.moduleName}.dialogs.clear-bounding-tile-selection.clear-button`
    ),
 ...
 */
export const shareSceneMedia = async (
  url,
  style,
  type = "image",
  loop = false,
  ...
  if (!canvas.scene) {
    return ui.notifications.warn(
      game.i18n.localize(`${constants.moduleName}.share.scene-no-scene`)
    );
  }
...
async function _promptShareBoundingTileSection(boundingTiles) {
  ...
  return dialog({
    ...
    title: game.i18n.localize(
      `${constants.moduleName}.dialogs.share-bounding-tile-selection.title`
    ),
    content,
    cancelLabel: game.i18n.localize(
      `${constants.moduleName}.dialogs.share-bounding-tile-selection.cancel-button`
    ),
    validateLabel: game.i18n.localize(
      `${constants.moduleName}.dialogs.share-bounding-tile-selection.share-button`
    ),
    validateCallback: (html) => {
      const boundingTileId = html
