...
export const findBoundingTileByName = (name) => {
  return canvas.scene.tiles.find(
    (t) =>
      t.flags?.[constants.moduleName]?.isBounding && t.flags?.[constants.moduleName]?.name === name
  );
};
...
export const chooseShareAction = async (url, title = "") => {
  const buttons = [
    ...
    },
    {
      ...
      label: game.i18n.localize(
        `${constants.moduleName}.dialogs.share-action.share-fullscreen-all`
      ),
    ...
    },
    {
      ...
      icon: '<i class="fas fa-expand-arrows-alt"></i>',
      label: game.i18n.localize(
        `${constants.moduleName}.dialogs.share-action.share-fullscreen-some`
      ),
  ...
  const content = await renderTemplate(
    `${constants.modulePath}/templates/choose-share-action-dialog.hbs`
  );
